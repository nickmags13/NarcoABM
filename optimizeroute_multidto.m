...
%         subroutepref(iactiveedges)=1-SLRISK(iactiveedges);
        ...
        newedgeparms=[];
        potsenders=unique(edgeparms(:,4:5)); %dto node index
...
%             potsenders=potsenders(ismember(potsenders,[1; dtorefvec(unique(edgeparms(:,4:5)))]));
%             ipotsenders=find(ismember(potsenders,dtorefvec(1:length(dtorefvec)-1)) == 1);
...
%             newedgeparms=[newedgeparms; (dtoADDVAL(ipotsenders,k)-...
%                dtoCTRANS(ipotsenders,k)) dtoSLRISK(ipotsenders,k) ...
%                ipotedge k*ones(length(ipotsenders),1)];
%         end
