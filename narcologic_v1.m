...
%         ipotnode=find(ismember(idptmnt,subinodepick)==1);
    ...
    noderoad=[noderoad; zeros(length(randnode),1)];
    if i == 1
...
%             end
            ...
            neihood=10000*ones(size(LANDSUIT));
            neihood(NEIHOOD{n,1})=NEIHOOD{n,2};     % find closet, contiguous cells
            neihood(isnan(mktacc) | sublu==iluse(1) | sublu== iluse(6))=10000;            
...
%                 1),'ascend');
            ...
            if lndrlosstol > abs(minsize(2)*LUPROD(NodeTable.Row(n),...
                    ...
                    length(find(sublu(IOWN{n,max(lt-1,1)}) == iluse(7))) >= minsize(10)
                % row crop land use
                ...
                pland(ismember(icropcells,IOWN{n,max(lt-1,1)}))=0;
            ...
            if lndrlosstol > abs(minsize(5)*LUPROD(NodeTable.Row(n),...
                ...
                pland(ismember(iplntcells,IOWN{n,max(lt-1,1)}))=0;
            ...
            if lndrlosstol > abs(minsize(7)*LUPROD(NodeTable.Row(n),...
                ...
                pland(ismember(ictlcells,IOWN{n,max(lt-1,1)}))=0;
            ...
            if isempty(subprod) == 1 || isempty(find(subprod(:,2) == maxlu,1)) == 1
                ...
                iadd2own=~ismember(potprod(imaxlu,1),IOWN{n,max(lt-1,1)});
                IOWN(n,lt)=mat2cell([IOWN{n,max(lt-1,1)}; potprod(imaxlu(iadd2own),...
                    1)],length(IOWN{n,max(lt-1,1)})+size(potprod(imaxlu(iadd2own),...
                    [1 2 4 5 6 7]),1));
            elseif isempty(find(subprod(:,2) == maxlu,1)) == 0
                ...
                iadd2own=~ismember(potprod(imaxlu(ichoose),1),IOWN{n,max(lt-1,1)});
                IOWN(n,lt)=mat2cell([IOWN{n,max(lt-1,1)}; ...
                    potprod(imaxlu(ichoose(iadd2own)),1)],length(IOWN{n,max(lt-1,1)})+...
