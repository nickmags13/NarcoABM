...
sl_max=135*ones(1,ERUNS);       %baseline; maximum interdiction capacity
sl_min=ceil(sl_max/6);          %baseline; minimum interdiction capacity
% sl_min=ceil(sl_max/2);

baserisk=0.7455*ones(1,ERUNS);     %baseline; threshold for risk premium, Caulkins et al. (1993)
...
sl_learn=0.4765*ones(1,ERUNS);     %baseline; rate of interdiction learning
rt_learn=0.1927*ones(1,ERUNS);     %baseline; rate of network agent learning

losslim=0.0119*ones(1,ERUNS);      %baseline; loss tolerance
...
targetseize=0.1481*ones(1,ERUNS);  %baseline; target portion of total flow to seize
...
expandmax=14*ones(1,ERUNS);     %number of new nodes established per month
